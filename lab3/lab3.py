...
# 1 слой
model.add(Dense(10, activation=activations.sigmoid, activity_regularizer='l2'))
...
model.add(Dropout(0.2))
# Выходной слой
model.add(Dense(1, activation=activations.sigmoid, activity_regularizer='l2'))

# Компилирование модели
# default value learning_rate = 0.001, rho = 0.9
model.compile(loss='binary_crossentropy', optimizer=optimizers.RMSprop(learning_rate=0.01, rho=0.99), metrics=['accuracy'])
# Обучение модели
#  epochs = 200
model.fit(x=x_train_scale, y=y_train_scale, epochs=1500, batch_size=10)
